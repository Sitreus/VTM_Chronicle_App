        ...
        <div style={{ display: "flex", gap: 30, flexWrap: "wrap" }}>
          {[
            ...
            <div key={s.label} style={{ textAlign: "center" }}>
              ...
              <div style={{ fontSize: 17, fontFamily: "'Cinzel', serif", letterSpacing: 2, color: "#a09888", textTransform: "uppercase" }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            {(cd.clocks || []).map(c => (
              <div key={c.id} style={{ ...S.card, flex: "0 0 auto", textAlign: "center", padding: 16, minWidth: 120 }}>
                ...
                <div style={{ fontFamily: "'Cinzel', serif", fontSize: 16, letterSpacing: 1, color: "#e8dcc6", marginTop: 8 }}>{c.name}</div>
        ...
        <div>
          ...
          {activeThreadsList.map(t => (
            <div key={t.id} style={{ ...S.card, padding: "12px 16px", marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <span style={{ fontFamily: "'Cinzel', serif", fontSize: 21, color: "#f0e6d4" }}>{t.title}</span>
                {t.description && <div style={{ fontSize: 20, color: "#b0a490", marginTop: 2 }}>{t.description}</div>}
              </div>
              <span style={{ ...S.tag(accent), fontSize: 13 }}>{t.type || "mystery"}</span>
            </div>
          ))}
        </div>
      )}
