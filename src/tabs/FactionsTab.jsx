  ...
  const influenceBar = (level) => {
    ...
    return (
      <div style={{ display: "flex", gap: 2, alignItems: "center" }}>
        ...
        <span style={{ fontSize: 15, color: "#a09888", marginLeft: 4 }}>{level}</span>
      </div>
    );
  };
            ...
            <div key={f.id} style={{ ...S.card, cursor: "pointer", position: "relative" }}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  ...
                  {f.territory && <div style={{ fontSize: 18, color: "#a09888", marginTop: 2 }}>📍 {f.territory}</div>}
                </div>
                <span style={{ ...S.tag(attitudeColor[f.attitude] || "#6a6a6a"), fontSize: 14 }}>{f.attitude}</span>
              </div>
              {f.description && <div style={{ marginTop: 8, fontSize: 21, color: "#d4c8ae", lineHeight: 1.5 }}>{f.description}</div>}
              <div style={{ marginTop: 8 }}>{influenceBar(f.influence || "None")}</div>
              {f.goals && (
                <div style={{ marginTop: 8, fontSize: 18, color: "#b0a490" }}>
                  <span style={{ color: accent, fontFamily: "'Cinzel', serif", fontSize: 15, letterSpacing: 1 }}>GOALS </span>
                  {f.goals}
                </div>
              )}
              {f.members?.length > 0 && (
                <div style={{ marginTop: 8, fontSize: 17, color: "#a09888" }}>
