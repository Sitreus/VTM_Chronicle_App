          ...
          {allLocs.map(loc => (
            <div key={loc.id} style={{ ...S.card, cursor: "pointer", position: "relative" }}
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                <span style={{ ...S.tag(accent), fontSize: 22 }}>{loc.type}</span>
                {loc.controlledBy && <span style={{ ...S.tag("#4a6a8a"), fontSize: 22 }}>{loc.controlledBy}</span>}
              </div>
              {loc.description && <div style={{ marginTop: 8, fontSize: 36, color: "#d4c8ae", lineHeight: 1.5 }}>{loc.description}</div>}
              {loc.atmosphere && (
                <div style={{ marginTop: 6, fontSize: 32, fontStyle: "italic", color: "#a89d8d" }}>
                  ✦ {loc.atmosphere}
                </div>
              )}
              {loc.secrets && (
                <div style={{ marginTop: 6, fontSize: 32, color: "#d4962e" }}>
                  🔒 {loc.secrets}
                </div>
              )}
              {loc.sessions?.length > 0 && (
                <div style={{ marginTop: 8, fontSize: 24, color: "#5a5a65", fontFamily: "'Cinzel', serif", letterSpacing: 1 }}>
