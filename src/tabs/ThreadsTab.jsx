        ...
        <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 24 }}>
          {clocks.map(c => (
            <div key={c.id} style={{ ...S.card, flex: "0 0 auto", textAlign: "center", padding: 16, minWidth: 130,
              ...
              <div style={{ fontFamily: "'Cinzel', serif", fontSize: 17, letterSpacing: 1, color: "#e8dcc6", marginTop: 8 }}>{c.name}</div>
              <div style={{ fontSize: 14, color: "#7a7068", textTransform: "uppercase", letterSpacing: 1 }}>{c.type || "threat"}</div>
              {c.filled >= c.segments && (
        ...
        ["active", "cold", "resolved"].map(status => {
          ...
          return (
            <div key={status} style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 15, fontFamily: "'Cinzel', serif", letterSpacing: 2, color: statusColor[status], textTransform: "uppercase", marginBottom: 8 }}>
              ...
              {group.map(t => (
                <div key={t.id} style={{ ...S.card, marginBottom: 8, position: "relative", borderLeft: `3px solid ${statusColor[t.status]}`,
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ fontFamily: "'Cinzel', serif", fontSize: 21, color: "#e8dcc6", letterSpacing: 1 }}>{t.title}</span>
                        <span style={{ ...S.tag(statusColor[t.status]), fontSize: 13 }}>{t.type}</span>
                      </div>
                      {t.description && <div style={{ fontSize: 20, color: "#b0a490", marginTop: 4 }}>{t.description}</div>}
                      {t.clues?.length > 0 && (
                        <div style={{ marginTop: 8 }}>
                          {t.clues.map((c, i) => (
                            <div key={i} style={{ fontSize: 18, color: "#c4b49e", marginBottom: 2 }}>
                              <span style={{ color: accent, fontSize: 16 }}>S{c.session}</span> {c.text}
                            </div>
                          ))}
                        </div>
                      )}
                  ...
                  {t.session && (
                    <div style={{ fontSize: 14, color: "#5a5a65", marginTop: 6, fontFamily: "'Cinzel', serif", letterSpacing: 1 }}>
