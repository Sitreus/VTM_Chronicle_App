    ...
    <div style={{ ...S.card, position: "relative" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          <div style={{ ...S.muted, fontSize: 17, marginTop: 2 }}>{session.date}</div>
        </div>
      ...
      {session.summary && (
        <div style={{ marginTop: 10, fontSize: 20, lineHeight: 1.6, color: "#d4c8ae" }}>
          {session.summary}
        </div>
      )}
      {session.storyBeats?.length > 0 && (
        <div style={{ marginTop: 10 }}>
          {session.storyBeats.map((b, i) => (
            <div key={i} style={{ fontSize: 18, color: "#c4b49e", marginBottom: 4 }}>
