...
export default function NPCCard({ npc, accent, onEdit, onDelete }) {
  const profileField = (icon, label, value) => value ? (
    <div style={{ marginTop: 8, fontSize: 36, lineHeight: 1.5, color: "#b0a490" }}>
      <span style={{ color: accent, fontSize: 28, fontFamily: "'Cinzel', serif", letterSpacing: 1, textTransform: "uppercase" }}>
        {icon} {label}
      </span>
    ...
    <div style={{ ...S.card, position: "relative", cursor: "pointer" }} onClick={() => onEdit(npc)}>
      <div style={{ display: "flex", gap: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {npc.faction && <div style={{ ...S.muted, fontSize: 34, marginTop: 2 }}>{npc.faction}</div>}
      ...
      {npc.description && (
        <div style={{ marginTop: 10, fontSize: 38, lineHeight: 1.5, color: "#d4c8ae" }}>
          {npc.description}
        </div>
      )}
      ...
      {npc.notes && (
        <div style={{ marginTop: 8, fontSize: 34, fontStyle: "italic", color: "#a09888" }}>
          ✦ {npc.notes}
        </div>
      )}
      {npc.lastSeen && (
        <div style={{ marginTop: 10, fontSize: 30, color: "#9a9aa5", fontFamily: "'Cinzel', serif", letterSpacing: 1 }}>
